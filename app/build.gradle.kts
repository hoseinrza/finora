...
// The Crashlytics Gradle plugin wires an uploadCrashlyticsMappingFileRelease task that hard-fails
// Gradle's task-input validation ("Input file does not exist") for the appId file the
// google-services plugin only writes when a real google-services.json is present - setting
// firebaseCrashlytics { mappingFileUploadEnabled = false } does NOT avoid this, because Gradle
// validates the task's declared @InputFile before the task's own enabled/onlyIf logic runs. The
// only reliable fix is to not apply the plugin at all (so the task is never created) when there's
// no google-services.json - CI's release workflow supplies a real one, so production builds still
// get mapping uploads.
